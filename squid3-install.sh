...
# Squid Installer
# Author: https://www.serverOk.in & TheVisual
# Email: info@serverOk.in
# Github: https://github.com/serverok/squid-proxy- // https://github.com/TheVisual/squid-proxy-installer
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 20.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/TheVisual/squid-proxy-installer/master/squid.conf
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/TheVisual/squid-proxy-installer/master/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 7777 -j ACCEPT
    /sbin/iptables-save
    ...
    echo "OS NOT SUPPORTED. Contact info@ServerOk.in or https://github.com/TheVisual to add support for your OS"
    exit 1;
fi